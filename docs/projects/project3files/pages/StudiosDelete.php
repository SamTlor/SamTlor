...
.browser > h2, p {
  ...
  font-size: 90%;
    ...
    <form method="post" action="" name="signup-form">
        ...
        <main>
        <h1>Enter the Studio ID to Delete the Studio</h2>
    ...
    <button type="submit" name="register" value="register">Delete Studio</button>
...
<?php
    ...
    if (isset($_POST['register'])) {
        ...
        if($result){
            $query = $conn->prepare("DELETE FROM Studios WHERE Studios_ID=:Studios_ID");
            $query->bindParam("Studios_ID", $Studios_ID, PDO::PARAM_STR);
            $query->execute();
            echo "<p>Studio sucessfully deleted</p>";
        }
        else {
            echo '<p class = "error">Studio is not in the database</p>';
        }
    }
