---
title: "Better Solar Wind Conditions at Psyche 16"
...
<div className="flex desktop:w-1/2 mobile:w-full mobile:p-4 flex-col desktop:p-20">
    ...
    I originally became interested in this project for my senior capstone project 2022-2023. 

    https://psyche.ssl.berkeley.edu/get-involved/capstone-projects/capstone-projects-silver-class/solar-wind-prediction-using-machine-learning-vcu-a/

    I didn't have a lot of experience with machine learning at that time and so in 2026 after taking courses at Georgia Tech Institute of Technology I decided to try again on my own. 

    The main goals were to predict the magnetic field and other solar wind conditions around the distance from the sun that psyche 16 is (2.53 AU / 235 million miles / 378 million km - 3.32 AU / 309 million miles / 497 million km). The main goal was also to see what happens when winds interact with each other. For example, what happens when a cluster of slow moving winds are hit with a gust of very fast moving winds.
    
    Ultimately, I was unsuccessful at finding specific features to provide insights to predict the wind conditions at that distance. That's not to say that the entire project is useless. Instead it shows definitively which factors are not useful for solving these problems. This problem is very complex and the resources that humanity as a whole has to solve them are very limited. 
